...
def extract_article(item: NewsItem) -> NewsItem:
    ...
    try:
        ...
        if item.content and not item.source.startswith("中新网"):
            candidates = [article.top_image, *list(article.images)]
...
def render_hot_topic(topic: HotTopic | str) -> str:
    if isinstance(topic, HotTopic):
        return (
            f'<li><details><summary><a href="{html.escape(topic.url, quote=True)}" target="_blank" rel="noopener">'
            f'{html.escape(topic.title)}</a></summary><p>{html.escape(topic.description)}</p></details></li>'
        )
    return f"<li>{html.escape(topic)}</li>"


def render_html(date_text: str, sections: dict[str, list[NewsItem]], hot_words: dict[str, list[HotTopic | str]], page_url: str) -> str:
    ...
    hot = "".join(
        ...
        f"{''.join(render_hot_topic(topic) for topic in words) or '<li>暂无数据</li>'}</ol>"
...
def build_wechat_messages(date_text: str, sections: dict[str, list[NewsItem]], hot_words: dict[str, list[HotTopic | str]], page_url: str) -> list[str]:
    messages, number = [], 0
    for section in SECTION_LIMITS:
    ...
    for name, words in hot_words.items():
        ...
        for index, topic in enumerate(words, start=1):
            if isinstance(topic, HotTopic):
                hot_lines.extend([f"{index}. [{topic.title}]({topic.url})", topic.description])
            else:
